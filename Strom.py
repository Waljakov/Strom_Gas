...
# Daten einlesen
daten = np.loadtxt("Stromzaehler.txt", dtype=(str), skiprows=1,)
datum = daten[:,0]
stand_all = daten[:,1]
stand_all = stand_all.astype(np.float)
dates_list_all=[datetime.datetime.strptime(dat, "%d.%m.%Y") for dat in datum]

# Variablen
StartMessung = 2 # Startmesspunkt der Auswahl
EndMessung = len(stand_all)-1 # Endmesspunkt der Auswahl
kosten_kWh = 0.2207 # kosten pro kWh in EUR
kosten_monat = 13.86 # Monatliche Fixkosten
Voranschlag_verbrauch = 2800 # Prognostizierter Jahresverbrauch in kWh
